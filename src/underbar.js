(function() {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var needsInitValue = arguments.length < 3;
    _.each(collection, function(element) {
      if (needsInitValue) {
        accumulator = element;
  ...
  _.some = function(collection, iterator) {
    ...
    var myIterator = hasIterator ? iterator : function(item) { return !!item; }
    return _.reduce(collection, function(passed, item) {
      if (passed) {
        return true;
      }
      return !!myIterator(item)
    }, false);
  };
  ...
  _.zip = function() {
    var args = Array.prototype.slice.call(arguments);
    var result = [];
    // TODO: First sort arg arrays by length before picking first one
    for (var i = 0; i < args[0].length; i++) {
      var currResult = [];
      for (var j = 0; j < args.length; j++) {
        currResult.push(args[j][i]);
      }
